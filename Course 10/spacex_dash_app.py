...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Get the total success and failed counts for all sites
        success_counts = spacex_df[spacex_df['class'] == 1]['class'].count()
        failed_counts = spacex_df[spacex_df['class'] == 0]['class'].count()
        labels = ['Success', 'Failed']
        values = [success_counts, failed_counts]
    else:
        # Get the success and failed counts for the selected site
        ...
        failed_counts = selected_site_df[selected_site_df['class'] == 0]['class'].count()
        labels = ['Success', 'Failed']
        values = [success_counts, failed_counts]
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(site_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title=f'Payload vs. Launch Outcome for {entered_site}')
        return fig
